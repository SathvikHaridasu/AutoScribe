...
class AutoScribe:
    def __init__(self):
        ...
        # Human-like typo schedule: every N words (random between min/max) — customizable
        self.typo_min_words = tk.IntVar(value=5)
        self.typo_max_words = tk.IntVar(value=12)
    ...
    # ---------- UI ----------
    def setup_ui(self):
        """Setup the user interface (simple, original layout)"""
        ...
        self.text_area = tk.Text(main_frame, height=10, width=40, wrap="word")
        ...
        ttk.Entry(min_frame, textvariable=self.min_wpm, width=5, justify="center").pack(side=tk.LEFT, padx=5)

        # Max WPM
        max_frame = ttk.Frame(wpm_frame)
        max_frame.pack(side=tk.LEFT)
        ttk.Label(max_frame, text="Max WPM:").pack(side=tk.LEFT)
        ttk.Entry(max_frame, textvariable=self.max_wpm, width=5, justify="center").pack(side=tk.LEFT, padx=5)

        # Validate WPM inputs
        def validate_wpm(*args):
            try:
                min_wpm_val = int(self.min_wpm.get())
                max_wpm_val = int(self.max_wpm.get())
                if min_wpm_val < 1:
                    min_wpm_val = 1
                if max_wpm_val < 1:
                    max_wpm_val = 1
                if min_wpm_val > max_wpm_val:
                    min_wpm_val = max_wpm_val
                self.min_wpm.set(min_wpm_val)
                self.max_wpm.set(max_wpm_val)
            except tk.TclError:
                pass
        self.min_wpm.trace('w', validate_wpm)
        self.max_wpm.trace('w', validate_wpm)

        # Mistake frequency frame (customizable)
        ...
        ttk.Entry(typo_row, textvariable=self.typo_min_words, width=5, justify="center").pack(side=tk.LEFT, padx=5)
        ttk.Label(typo_row, text="to").pack(side=tk.LEFT)
        ttk.Entry(typo_row, textvariable=self.typo_max_words, width=5, justify="center").pack(side=tk.LEFT, padx=5)
        ttk.Label(typo_row, text="words (random)").pack(side=tk.LEFT, padx=(5, 0))

        # Validate typo frequency inputs
        def validate_typo(*args):
            try:
                mn = int(self.typo_min_words.get())
                mx = int(self.typo_max_words.get())
                if mn < 1:
                    mn = 1
                if mx < 1:
                    mx = 1
                if mn > mx:
                    mn = mx
                self.typo_min_words.set(mn)
                self.typo_max_words.set(mx)
            except tk.TclError:
                pass
        self.typo_min_words.trace('w', validate_typo)
        self.typo_max_words.trace('w', validate_typo)
    ...
    def calculate_delay(self):
        ...
        min_wpm = max(1, self.min_wpm.get())
        max_wpm = max(min_wpm, self.max_wpm.get())
    ...
    def start_typing(self):
        ...
        # Reset typo schedule based on current UI values
        self.words_since_last_typo = 0
        mn = max(1, self.typo_min_words.get())
        mx = max(mn, self.typo_max_words.get())
        self.typo_min_words.set(mn)
        self.typo_max_words.set(mx)
        self.next_typo_after_words = random.randint(mn, mx)
    ...
    def type_text(self):
        ...
        while self.current_index < len(self.text_to_type) and self.typing:
            ...
            ch = self.text_to_type[self.current_index]

            if is_word_char(ch):
                # Starting a new word? Decide if this word should get a typo based on schedule
                ...
                typed_word_len += 1
                current_word_correct.append(ch)
